...
public class BbsController {
	...
	/*
	 ...
	 객체들은 아래와같이 자동주입 받아서 사용하는 것이 스프링
	 개발방법론에 부합하는 방법이다.
	 */
	@Autowired
	...
	/*
	  ...
	  template; System.out.println("@Autowired->JDBCTemplate 연결성공");
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		 ...
		 호출하고, 실제 DAO의 호출이나 비즈니스 호출은 아래 Command객체가
		 처리하게된다.
		 */
	...
	public String passwordAction(Model model, HttpServletRequest req) {
		...
		if(rowExist<=0) {
			...
			if(mode.equals("edit")) {
				...
				/*
				 컨트롤러에서 뷰의 경로를 변환할 때 아래와 같이 redirect
