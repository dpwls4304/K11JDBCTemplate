...
/*
 ...
 -Object queryForObject(String sql, RowMapper rowMapper) 혹은
...
public class JDBCTemplateDAO {
	...
	public int password(String idx, String pass) {
		 ...
		 */
		try {
			SpringBbsDTO dto = template.queryForObject(
					sql, new BeanPropertyRowMapper<SpringBbsDTO>(
							SpringBbsDTO.class));
			retNum = dto.getIdx();
	...
	public void reply(final SpringBbsDTO dto) {
		...
		//답변글쓰기전 레코드 업데이트
		rereplyPrevUpdate(dto.getBgroup(), dto.getBstep());
		
		/*
		 원본글의 경우 idx의 bgroup은 동일한 값을 입력함.
	...
	public void rereplyPrevUpdate(final int strGroup, final int strStep) {
		String sql = "UPDATE springboard "
				+ "set bstep = bstep+1 "
